...
def get_low_quality_image(test_loader):
    ...
    low_quality_image = F.interpolate(image.unsqueeze(0), size=(14, 14), mode='bilinear', align_corners=False).squeeze(0)
    
    return low_quality_image, label

def finish_low_quality_image(model, low_quality_image, device, partial_start=40.0, steps=None):
    """
    ...
    upscaled_image = F.interpolate(low_quality_image.unsqueeze(0), size=(28, 28), mode='bilinear', align_corners=False).squeeze(0)
...
def main(args):
    ...
    save_image(low_quality_image, os.path.join(args.output_dir, f"{args.prefix}low_quality_image.png"))
    ...
    save_image(finished_image, os.path.join(args.output_dir, f"{args.prefix}finished_image.png"))
